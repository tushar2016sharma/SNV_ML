...
# PREPROCESS DATA
def preprocess_data(var_df, ref_df, y_arr, explained_variance_threshold=None, apply_svd=False):
...
# CNN HyperModel for Keras Tuner
class CNNHyperModel(kt.HyperModel):
    def build(self, hp):
        ...
        model.add(layers.Dense(hp.Int("dense_units_1", 64, 512, step=64), activation="relu"))
        model.add(layers.BatchNormalization())
        model.add(layers.Dropout(hp.Float("dropout_fc", 0.2, 0.5, step=0.1)))
...
def tune_and_train(X_train, y_train, X_val, y_val, models_dir, sample_id):
    ...
    model.fit(X_train, y_train, validation_data=(X_val, y_val), epochs=150, callbacks=[stop_early], verbose=2)

    best_model_path = os.path.join(models_dir, f"{sample_id}_CNN_model.h5")
...
def evaluate_model(model, X_test, y_test, sample_id, output_dir):
    ...
    image_path = os.path.join(output_dir, f'{sample_id}_CNN_conf_mat.png')
...
for sample_id in sample_ids:
    ...
    X_train_combined_final, X_val_combined_final, X_test_combined_final, y_train, y_val, y_test = preprocess_data(
        var_df, ref_df, y_arr
    )

    output_dir = os.path.join(results_dir, f"hyperband_CNN_{sample_id}")
...
results_csv_path = os.path.join(results_dir, "cnn_hyperband_results.csv")
results_df.to_csv(results_csv_path, index=False)
print("All results of CNN saved to 'results/' directory.")
